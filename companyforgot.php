    ...
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'request_reset') {
            ...
            if (empty($email)) {
                ...
                // Check if email exists in company_supervisors database
                $query = "SELECT * FROM company_supervisors WHERE email = ?";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    if ($user['account_status'] === 'Inactive') {
                        ...
                        $clear_query = "UPDATE company_supervisors SET reset_token = NULL, reset_expires = NULL WHERE email = ?";
                        ...
                        $update_query = "UPDATE company_supervisors SET reset_token = ?, reset_expires = ? WHERE email = ?";
                        ...
                        if (mysqli_stmt_execute($update_stmt)) {
                            ...
                            $verify_query = "SELECT reset_token FROM company_supervisors WHERE email = ?";
                            $verify_stmt = mysqli_prepare($conn, $verify_query);
                            mysqli_stmt_bind_param($verify_stmt, "s", $email);
                            mysqli_stmt_execute($verify_stmt);
                            $verify_result = mysqli_stmt_get_result($verify_stmt);
                            $verify_data = mysqli_fetch_assoc($verify_result);
                            error_log("Saved to DB: '" . $verify_data['reset_token'] . "' (length: " . strlen($verify_data['reset_token']) . ")");
                            ...
                            try {
                                ...
                                $mail->isHTML(true);
                                $mail->Subject = 'Password Reset Request - OnTheJob Tracker Company Portal';
                                ...
                                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                                    <div style="text-align: center; margin-bottom: 30px;">
                                        ...
                                        <p style="color: #666; margin: 5px 0;">Company Supervisor Portal - BULSU OJT Monitoring System</p>
                                    </div>
                                    
                                    <h2 style="color: #333;">Password Reset Request</h2>
                                    <p style="color: #555; line-height: 1.6;">
                                        Hello ' . htmlspecialchars($user['full_name']) . ',<br><br>
                                        We received a request to reset your password for your Company Supervisor account. If you made this request, please use the verification code below to reset your password.
                                    </p>
                                    ...
                                    <div style="margin: 30px 0;">
                                        ...
                                        <ul style="color: #555; line-height: 1.8;">
                                            ...
                                            <li><strong>Company Supervisor:</strong> ' . htmlspecialchars($user['full_name']) . '</li>
                                ...
                                $_SESSION['company_reset_email'] = $email;
                                $_SESSION['company_reset_requested'] = true;
                                
                                header("Location: companyforgot.php?step=verify");
                                exit;
                    ...
                    $error = "No company supervisor account found with this email address.";
                }
            }
        }
        ...
        elseif ($_POST['action'] === 'verify_code') {
            ...
            $email = $_SESSION['company_reset_email'] ?? '';
            
            if (empty($reset_code)) {
                $error = "Please enter the verification code.";
            } elseif (empty($email)) {
                $error = "Session expired. Please start over.";
                header("Location: companyforgot.php?step=request");
                exit;
            } else {
                ...
                $query = "SELECT reset_token, reset_expires FROM company_supervisors WHERE email = ?";
                $stmt = mysqli_prepare($conn, $query);
                mysqli_stmt_bind_param($stmt, "s", $email);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    $db_code = trim($user_data['reset_token']); // Trim whitespace
                    $expiry_time = $user_data['reset_expires'];
                    ...
                    if ($reset_code === $db_code) {
                        ...
                        if ($current_time <= $expiry_time) {
                            ...
                            $_SESSION['company_code_verified'] = true;
                            error_log("Code verification successful for $email");
                            header("Location: companyforgot.php?step=reset");
                            exit;
                        } else {
        ...
        elseif ($_POST['action'] === 'reset_password') {
            ...
            if (empty($new_password) || empty($confirm_password)) {
                ...
                $update_query = "UPDATE company_supervisors SET password = ?, reset_token = NULL, reset_expires = NULL WHERE email = ?";
                $update_stmt = mysqli_prepare($conn, $update_query);
                mysqli_stmt_bind_param($update_stmt, "ss", $hashed_password, $email);
                
                if (mysqli_stmt_execute($update_stmt)) {
                    // Clear session data
                    unset($_SESSION['company_reset_email']);
                    unset($_SESSION['company_reset_requested']);
                    unset($_SESSION['company_code_verified']);
                    
                    // Set success flag instead of using alert
                    $_SESSION['company_password_reset_success'] = true;
                    header("Location: companyforgot.php?step=success");
                    exit;
                } else {
        ...
        if ($step === 'success' && isset($_SESSION['company_password_reset_success'])) {
            unset($_SESSION['company_password_reset_success']);
        ...
        elseif ($_POST['action'] === 'resend_code') {
            ...
            if (mysqli_stmt_execute($update_stmt)) {
                ...
                $verify_result = mysqli_stmt_get_result($verify_stmt);
                $verify_data = mysqli_fetch_assoc($verify_result);
                error_log("New code saved to DB: '" . $verify_data['reset_token'] . "'");
                
                // Get user data for email
                $user_query = "SELECT * FROM company_supervisors WHERE email = ?";
                ...
                try {
                    ...
                    $mail->isHTML(true);
                    $mail->Subject = 'New Password Reset Code - OnTheJob Tracker Company Portal';
...
if ($step === 'verify' && !isset($_SESSION['company_reset_requested'])) {
    header("Location: companyforgot.php?step=request");
    exit;
}

if ($step === 'reset' && !isset($_SESSION['company_code_verified'])) {
    header("Location: companyforgot.php?step=request");
    exit;
}
?>
...
<head>
    ...
    <title>
        ...
        ?> - BULSU OnTheJob Tracker Company Portal
    </title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-gray-100 text-gray-800 min-h-screen flex flex-col">
    <header class="bg-gradient-to-r from-bulsu-maroon to-bulsu-dark-maroon text-white shadow-lg">
        <nav class="max-w-6xl mx-auto flex items-center justify-between px-4 py-4 relative">
            ...
            <div class="hidden md:flex space-x-4">
                <a href="supervisor-login.php" class="bg-bulsu-gold bg-opacity-20 border border-bulsu-gold border-opacity-50 rounded px-4 py-2 font-medium hover:bg-opacity-30 transition text-bulsu-gold">Company Login</a>
                <a href="login.php" class="bg-bulsu-gold text-bulsu-maroon rounded px-4 py-2 font-medium hover:bg-yellow-400 transition">Student Login</a>
            </div>
            <!-- Mobile Menu -->
            <div id="mobile-menu" class="md:hidden hidden absolute top-full left-0 w-full bg-bulsu-maroon z-50 px-4 pb-4 shadow-lg">
                ...
                <div class="flex flex-col space-y-2 mt-4">
                    <a href="supervisor-login.php" class="bg-bulsu-gold bg-opacity-20 border border-bulsu-gold border-opacity-50 rounded px-4 py-2 font-medium text-bulsu-gold hover:bg-opacity-30 transition">Company Login</a>
                    <a href="login.php" class="bg-bulsu-gold text-bulsu-maroon rounded px-4 py-2 font-medium hover:bg-yellow-400 transition text-center">Student Login</a>
                </div>
            </div>
        </nav>
    </header>
        ...
        <div class="bg-white bg-opacity-95 rounded-2xl shadow-2xl p-8 md:p-12 max-w-lg w-full animate-fadeInUp">
            ...
            <div class="mb-8">
                <div class="mb-6 text-center">
                    ...
                    <h4 class="text-bulsu-maroon font-medium text-base mb-2">Company Supervisor Portal</h4>
            ...
            <?php if ($step === 'request'): ?>
                ...
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-600">Enter your company email address and we'll send you a verification code to reset your supervisor password.</p>
                </div>
                <form method="post" class="space-y-6">
                    <input type="hidden" name="action" value="request_reset">
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Company Email Address</label>
                        <input type="email" id="email" name="email" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-bulsu-gold focus:border-bulsu-gold" placeholder="Enter your registered company email address" required>
                    </div>
                ...
                <div class="text-center mt-6">
                    <a href="supervisor-login.php" class="text-bulsu-maroon hover:text-bulsu-dark-maroon font-medium">← Back to Company Login</a>
                </div>
            ...
            <?php elseif ($step === 'verify'): ?>
                ...
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-600">We've sent a 6-digit verification code to your company email address. Please enter it below.</p>
                </div>
                <form method="post" class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-2">Enter the 6-digit code sent to your company email</p>
                    </div>
                    <button type="submit" class="w-full bg-gradient-to-r from-bulsu-maroon to-bulsu-dark-maroon text-white py-3 rounded-lg font-semibold hover:from-bulsu-dark-maroon hover:to-bulsu-maroon transition duration-300 transform hover:scale-105">
                        Verify Code
                    </button>
                ...
                <div class="text-center mt-4">
                    <a href="companyforgot.php?step=request" class="text-gray-500 hover:text-gray-700">← Start Over</a>
                </div>
            ...
            <?php elseif ($step === 'reset'): ?>
                ...
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-600">Enter your new company supervisor password below. Make sure it's strong and secure.</p>
                </div>
                <form method="post" class="space-y-6">
            ...
            <?php elseif ($step === 'success'): ?>
                ...
                <div class="text-center">
                    <div class="mb-6">
                        ...
                        <p class="text-gray-600">Your company supervisor password has been successfully reset. You can now login with your new password.</p>
                    </div>
                    <a href="supervisor-login.php" class="inline-block bg-gradient-to-r from-bulsu-maroon to-bulsu-dark-maroon text-white px-8 py-3 rounded-lg font-semibold hover:from-bulsu-dark-maroon hover:to-bulsu-maroon transition duration-300 transform hover:scale-105">
                        Go to Company Login
                    </a>
                </div>
            <?php endif; ?>
        ...
        <div class="max-w-6xl mx-auto px-4">
            <div class="grid md:grid-cols-3 gap-8">
                <div>
                    ...
                    <p class="text-gray-300">BULSU AI-Powered Company OJT Performance Monitoring System</p>
                </div>
                <div>
                    <h4 class="font-semibold mb-3">Quick Links</h4>
                    <ul class="space-y-2">
                        <li><a href="index.php" class="text-gray-300 hover:text-bulsu-gold transition">Home</a></li>
                        <li><a href="supervisor-login.php" class="text-gray-300 hover:text-bulsu-gold transition">Company Login</a></li>
                        <li><a href="login.php" class="text-gray-300 hover:text-bulsu-gold transition">Student Login</a></li>
                    </ul>
                </div>
                <div>
                    <h4 class="font-semibold mb-3">Contact Info</h4>
                    <div class="text-gray-300 space-y-2">
                        <p>📧 company-support@bulsu-ojt.edu.ph</p>
                        <p>📞 (044) 760-0000</p>
            ...
            <div class="border-t border-gray-600 mt-8 pt-6 text-center">
                <p class="text-gray-400">&copy; <?php echo date('Y'); ?> Bulacan State University - Company Supervisor Portal. All rights reserved.</p>
            </div>
        </div>
    </footer>
    ...
    <script>
        ...
        <?php if ($step === 'success' && isset($_SESSION['company_password_reset_success'])): ?>
        // You can add a success animation or modal here if needed
        console.log('Company password reset successful!');
        <?php endif; ?>
    </script>
</body>
</html>