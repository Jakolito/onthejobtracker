    ...
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'request_reset') {
            ...
            if (empty($email)) {
                ...
                // Check if email exists in Academic_Adviser table
                $query = "SELECT * FROM Academic_Adviser WHERE email = ?";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    if ($user['status'] === 'inactive') {
                        ...
                        $clear_query = "UPDATE Academic_Adviser SET reset_code = NULL, reset_code_expiry = NULL WHERE email = ?";
                        ...
                        error_log("Generated reset code for adviser $email: '$reset_code' (length: " . strlen($reset_code) . ")");
                        
                        // Update database with reset code (store as VARCHAR/TEXT)
                        $update_query = "UPDATE Academic_Adviser SET reset_code = ?, reset_code_expiry = ? WHERE email = ?";
                        ...
                        if (mysqli_stmt_execute($update_stmt)) {
                            ...
                            $verify_query = "SELECT reset_code FROM Academic_Adviser WHERE email = ?";
                            ...
                            try {
                                ...
                                $mail->isHTML(true);
                                $mail->Subject = 'Password Reset Request - OnTheJob Tracker (Academic Adviser)';
                                
                                // Debug: Log the code being sent in email
                                error_log("Sending in email to adviser: '$reset_code'");
                                
                                $mail->Body = '
                                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                                    <div style="text-align: center; margin-bottom: 30px;">
                                        <h1 style="color: #800000; margin: 0;">OnTheJob Tracker</h1>
                                        <p style="color: #666; margin: 5px 0;">BULSU Academic Adviser Portal</p>
                                    </div>
                                    
                                    <h2 style="color: #333;">Password Reset Request</h2>
                                    <p style="color: #555; line-height: 1.6;">
                                        Hello ' . htmlspecialchars($user['name']) . ',<br><br>
                                        We received a request to reset your password for your OnTheJob Tracker Academic Adviser account. If you made this request, please use the verification code below to reset your password.
                                    </p>
                                    ...
                                    <div style="margin: 30px 0;">
                                        ...
                                        <ul style="color: #555; line-height: 1.8;">
                                            ...
                                            <li><strong>Adviser ID:</strong> ' . htmlspecialchars($user['id']) . '</li>
                                ...
                                $_SESSION['admin_reset_requested'] = true;
                                
                                header("Location: adminforgot.php?step=verify");
                                exit;
                                
                            } catch (Exception $e) {
                                $error = "Failed to send reset email. Please try again later.";
                                error_log("Adviser password reset email failed: " . $e->getMessage());
                            }
                        } else {
                    ...
                    $error = "No academic adviser account found with this email address.";
                }
            }
        }
        ...
        elseif ($_POST['action'] === 'verify_code') {
            ...
            if (empty($reset_code)) {
                ...
                header("Location: adminforgot.php?step=request");
                exit;
            } else {
                // Debug: Log what we're comparing
                error_log("Verifying adviser code for $email. Input: '$reset_code' (length: " . strlen($reset_code) . ")");
                
                // Get current timestamp in same format as database
                $current_time = date('Y-m-d H:i:s');
                
                // First, get the adviser's reset data
                $query = "SELECT reset_code, reset_code_expiry FROM Academic_Adviser WHERE email = ?";
                $stmt = mysqli_prepare($conn, $query);
                mysqli_stmt_bind_param($stmt, "s", $email);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    // Debug logging
                    error_log("Adviser DB Code: '$db_code' (length: " . strlen($db_code) . ")");
                    ...
                    if ($reset_code === $db_code) {
                        ...
                        if ($current_time <= $expiry_time) {
                            ...
                            error_log("Adviser code verification successful for $email");
                            header("Location: adminforgot.php?step=reset");
                            exit;
                        } else {
                            // Code is correct but expired
                            $error = "Verification code has expired. Please request a new one.";
                            error_log("Adviser code expired for $email. Expiry: $expiry_time, Current: $current_time");
                        }
                    } else {
                        // Code doesn't match
                        $error = "Invalid verification code. Please check and try again.";
                        error_log("Adviser code mismatch for $email. Expected: '$db_code', Got: '$reset_code'");
                    }
                } else {
                    $error = "No reset request found. Please start over.";
                    error_log("No adviser reset data found for $email");
                }
            }
        }
        ...
        elseif ($_POST['action'] === 'reset_password') {
            ...
            if (empty($new_password) || empty($confirm_password)) {
                ...
                $update_query = "UPDATE Academic_Adviser SET password = ?, reset_code = NULL, reset_code_expiry = NULL WHERE email = ?";
                $update_stmt = mysqli_prepare($conn, $update_query);
                ...
                if (mysqli_stmt_execute($update_stmt)) {
                    ...
                    unset($_SESSION['admin_reset_requested']);
                    unset($_SESSION['admin_code_verified']);
                    
                    // Set success flag instead of using alert
                    $_SESSION['admin_password_reset_success'] = true;
                    header("Location: adminforgot.php?step=success");
                    exit;
                } else {
                    $error = "Failed to reset password. Please try again.";
                    error_log("Adviser password reset failed: " . mysqli_error($conn));
                }
            }
        }
        
        if ($step === 'success' && isset($_SESSION['admin_password_reset_success'])) {
        ...
        elseif ($_POST['action'] === 'resend_code') {
            ...
            error_log("Generated new adviser reset code for $email: '$reset_code' (length: " . strlen($reset_code) . ")");
            ...
            if (mysqli_stmt_execute($update_stmt)) {
                ...
                $verify_result = mysqli_stmt_get_result($verify_stmt);
                $verify_data = mysqli_fetch_assoc($verify_result);
                error_log("New adviser code saved to DB: '" . $verify_data['reset_code'] . "'");
                
                // Get user data for email
                $user_query = "SELECT * FROM Academic_Adviser WHERE email = ?";
                ...
                try {
                    ...
                    $mail->isHTML(true);
                    $mail->Subject = 'New Password Reset Code - OnTheJob Tracker (Academic Adviser)';
                    
                    // Debug: Log the code being sent
                    error_log("Sending new adviser code in email: '$reset_code'");
                    ...
                    error_log("Adviser resend email failed: " . $e->getMessage());
                }
            } else {
                $error = "Failed to generate new code. Please try again.";
                error_log("Database update failed for adviser resend: " . mysqli_error($conn));
            }
        }
    }
}
...
if ($step === 'verify' && !isset($_SESSION['admin_reset_requested'])) {
    header("Location: adminforgot.php?step=request");
    exit;
}

if ($step === 'reset' && !isset($_SESSION['admin_code_verified'])) {
    header("Location: adminforgot.php?step=request");
    exit;
}
?>
...
<head>
    ...
    <title>
        <?php 
        if ($step === 'request') echo 'Forgot Password - Academic Adviser';
        elseif ($step === 'verify') echo 'Verify Reset Code - Academic Adviser';
...
<body class="bg-gray-100 text-gray-800 min-h-screen flex flex-col">
        ...
        <div class="bg-white bg-opacity-95 rounded-2xl shadow-2xl p-8 md:p-12 max-w-lg w-full animate-fadeInUp">
            ...
            <div class="mb-8">
                <div class="mb-6 text-center">
                    ...
                    <div class="inline-block bg-gradient-to-r from-bulsu-gold to-yellow-500 text-bulsu-maroon px-4 py-2 rounded-full font-semibold mt-4">Academic Adviser Portal</div>
                </div>
            ...
            <?php if ($step === 'request'): ?>
                ...
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-600">Enter your academic email address and we'll send you a verification code to reset your password.</p>
                </div>
                <form method="post" class="space-y-6">
                    <input type="hidden" name="action" value="request_reset">
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Academic Email Address</label>
                        <input type="email" id="email" name="email" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-bulsu-gold focus:border-bulsu-gold" placeholder="Enter your academic email address" required>
                    </div>
                ...
                <div class="text-center mt-6">
                    <a href="adviser-login.php" class="text-bulsu-maroon hover:text-bulsu-dark-maroon font-medium">← Back to Adviser Login</a>
                </div>
            ...
            <?php elseif ($step === 'verify'): ?>
                ...
                <div class="text-center mt-4">
                    <a href="adminforgot.php?step=request" class="text-gray-500 hover:text-gray-700">← Start Over</a>
                </div>
            ...
            <?php elseif ($step === 'success'): ?>
                ...
                <div class="text-center">
                    ...
                    <a href="adviser-login.php" class="inline-block bg-gradient-to-r from-bulsu-maroon to-bulsu-dark-maroon text-white px-8 py-3 rounded-lg font-semibold hover:from-bulsu-dark-maroon hover:to-bulsu-maroon transition duration-300 transform hover:scale-105">
                        Go to Adviser Login
                    </a>
                </div>
            <?php endif; ?>
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        ...
                        <li><a href="adviser-login.php" class="text-gray-300 hover:text-bulsu-gold transition">Adviser Login</a></li>
                    </ul>
                </div>
                <div>
    ...
    <script>
        ...
        <?php if ($step === 'success' && isset($_SESSION['admin_password_reset_success'])): ?>
        // You can add a success animation or modal here if needed
        console.log('Academic Adviser password reset successful!');
        <?php endif; ?>
    </script>
</body>
</html>